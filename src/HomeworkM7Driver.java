...
public class HomeworkM7Driver {
	...
	public static boolean qNotFollowedByU(String word) {
		...
		return qNFUHelper(word,word.length(), returnBool);
	}

	private static boolean qNFUHelper(String word, int testLength, boolean returnBool){
		if(testLength==0){
			return returnBool;
		}
		if(testLength==1){
			if(word.substring(word.length()-testLength).equalsIgnoreCase("q")){
				return true;
			}
			else{
				return returnBool;
		...
		if(word.substring(word.length()-testLength,word.length()-testLength+1).equalsIgnoreCase("q")&& !word.substring(word.length()-testLength+1,word.length()-testLength+2).equalsIgnoreCase("u")){
			returnBool=true;
		}
		return qNFUHelper(word,testLength-1,returnBool);
	}


	public static void arrayReverse(int[] array) {
		aRevHelper(array, array.length/2,0); //size 4 array
	}

	private static void aRevHelper(int[] array,int increments,int currentInc){ //increments will start at 2
		if(currentInc==increments){
			return;
		}
		else{
			int temp = array[currentInc];
			array[currentInc]=array[array.length-currentInc-1];
			array[array.length-currentInc-1] = temp;
			aRevHelper(array,increments,currentInc+1);
		}
	...
	public static int countPositives(Multiset<Integer> set) {
		if (set.isEmpty()) {
			...
			int test = set.remove();
			int occurences = countPositives(set);
