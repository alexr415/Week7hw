...
//	}
	...
	public int getOccurrencesOf(T element) {
		Node current = head;
		return gOccOfHelper(head, element,0);

	}

	private int gOccOfHelper(Node current,T element, int occurences){
		if(current==null){
			return occurences;
		}
		if(current.data.equals(element))occurences++;
		return gOccOfHelper(current.next,element,occurences);
	}
